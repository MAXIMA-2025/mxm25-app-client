...
const organizations = {
    ...
    "Fakultas": {
        "Fakultas Ilmu Komunikasi": [
            "Ikatan Mahasiswa Ilmu Komunikasi (IM'KOM)"
        ],
    ...
    "Komunitas": [
        "UMN Gate",
        "POPSICLE",
        "Kelompok Studi Pasar Modal (KSPM) UMN"
    ]
};
...
const GamePage: React.FC = () => {
    const [progress, setProgress] = useState<boolean[]>(() => {
        try {
            ...
            return savedProgress ? JSON.parse(savedProgress) : new Array(allOrganizations.length).fill(false);
        } catch (error) {
            ...
            <div style={{ marginTop: '20px' }}>
                {Object.keys(organizations).map(categoryName => (
                    <div key={categoryName} style={{ marginBottom: '20px' }}>
                        ...
                        {Object.values(organizations[categoryName]).flat().map((organisator, index) => (
                            <div key={organisator} style={{ marginBottom: '10px' }}>
                                <input
                                    type="checkbox"
                                    checked={progress[allOrganizations.indexOf(organisator)]}
                                    onChange={() => handleToggleProgress(allOrganizations.indexOf(organisator))}
                                />
                                <label style={{ marginLeft: '10px' }}>
                                    {organisator} - {progress[allOrganizations.indexOf(organisator)] ? 'Selesai' : 'Belum Selesai'}
                                </label>
                            </div>
                        ))}
                    </div>
